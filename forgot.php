...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM students WHERE email='{$email}'")) > 0) {
        $query = mysqli_query($conn, "UPDATE students SET code='{$code}' WHERE email='{$email}'");
        ...
        if ($query) {        
            ...
            try {
                ...
                $mail->Username   = 'abairazakov09@gmail.com';                     //SMTP username
                $mail->Password   = '247128abai';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('abairazakov09@gmail.com');
                $mail->addAddress($email);

                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'no reply';
                $mail->Body    = 'Here is the verification link <b><a href="http://localhost/sign/?reset='.$code.'">http://localhost/sign/?reset='.$code.'</a></b>';
